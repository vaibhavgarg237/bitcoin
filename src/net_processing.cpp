...
static void UpdatePreferredDownload(CNode* node, CNodeState* state) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    state->fPreferredDownload = (!node->fInbound || node->HasPermission(PF_NOBAN)) && !node->fScout && !node->fClient;
...
bool ProcessMessage(CNode* pfrom, const std::string& msg_type, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, ChainstateManager& chainman, CTxMemPool& mempool, CConnman* connman, BanMan* banman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERSION) {
        ...
        if ((pfrom->m_conn_type == ConnectionType::OUTBOUND || pfrom->m_conn_type == ConnectionType::BLOCK_RELAY || pfrom->m_conn_type == ConnectionType::SCOUT) && !HasAllDesirableServiceFlags(nServices))
        {
        ...
        if (!pfrom->fInbound && pfrom->IsAddrRelayPeer())
        {
            ...
            if (pfrom->fScout || pfrom->nVersion >= CADDR_TIME_VERSION || connman->GetAddressCount() < 1000)
            {
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        bool fFetch = state.fPreferredDownload || (nPreferredDownload == 0 && !pto->fClient && !pto->fScout); // Download if this is a nice peer, or we have no nice peers and this one might do.
