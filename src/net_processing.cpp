        ...
        std::vector<CInv> vInv;
        {
            ...
            if (pto->m_tx_relay != nullptr) {
                ...
                if (gArgs.GetArg("-rebroadcast", DEFAULT_REBROADCAST_ENABLED) && pto->m_next_rebroadcast < current_time) {
                    ...
                    if (fSkipRun) {
                        LogPrint(BCLog::NET, "ABCD Bumping rebroadcast time because no new blocks since last cache run\n");
                    ...
                    if (!fFirst && !fSkipRun) {
                        ...
                        auto num = pto->m_tx_relay->setInventoryTxToSend.size();
                        pto->m_tx_relay->setInventoryTxToSend.insert(rebroadcastTxs.begin(), rebroadcastTxs.end());
                        auto delta = pto->m_tx_relay->setInventoryTxToSend.size() - num;
                        LogPrint(BCLog::NET, "ABCD setInventoryTxToSend changed by %d for peer=%d\n", delta, pto->GetId());
                    }
                }
